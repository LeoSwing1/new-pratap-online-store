...
/* API routing: when the customer store is opened with VS Code Live Server (e.g. :3000),
   keep the central Commerce API on the Node server (:8787). Production/server-served
   builds remain same-origin. */
const NPT_API_BASE=(()=>{
  if(window.NPT_API_BASE) return String(window.NPT_API_BASE).replace(/\/$/,'');
  const host=location.hostname;
  const devHost=host==='localhost'||host==='127.0.0.1';
  if(devHost && location.port && location.port!=='8787') return `${location.protocol}//${host}:8787`;
  return '';
})();
window.NPT_API_BASE=NPT_API_BASE;
const PAGE_SIZE=8;
function save(){NPT.save(S);localStorage.setItem('npt_cart',JSON.stringify(cart));updateCart()}
async function syncLiveCatalog(render=true){try{const r=await fetch(NPT_API_BASE+'/api/store/state',{cache:'no-store'});const d=await r.json();if(!d.ok||!Array.isArray(d.products))return false;const changed=JSON.stringify(S.products)!==JSON.stringify(d.products);S.products=d.products;if(Array.isArray(d.banners))S.banners=d.banners;save();if(changed){catNav();if(render||document.visibilityState==='visible')renderHome();}return true}catch(e){return false}}
async function refreshCustomerProfile(){const c=JSON.parse(localStorage.getItem('npt_customer')||'null');if(!c)return;try{const qs=new URLSearchParams();if(c.mobile)qs.set('mobile',c.mobile);else if(c.email)qs.set('email',c.email);const r=await fetch(NPT_API_BASE+'/api/store/customer?'+qs.toString(),{cache:'no-store'});const d=await r.json();if(d.ok&&d.customer){localStorage.setItem('npt_customer',JSON.stringify(d.customer));S.customers=S.customers||[];const i=S.customers.findIndex(x=>String(x.mobile||'').replace(/\D/g,'')===String(d.customer.mobile||'').replace(/\D/g,''));if(i>=0)S.customers[i]=d.customer;else S.customers.push(d.customer);save();document.getElementById('accountText').textContent=(d.customer.name||'Account').split(' ')[0]}else if(r.status===404){localStorage.removeItem('npt_customer');document.getElementById('accountText').textContent='Account'}}catch(e){}}
let customerSyncBusy=false,lastCustomerSyncAt=0;
async function syncLiveCustomer(renderOrders=false){const c=JSON.parse(localStorage.getItem('npt_customer')||'null');if(!c||customerSyncBusy)return false;customerSyncBusy=true;try{const qs=new URLSearchParams();if(c.mobile)qs.set('mobile',c.mobile);if(c.email)qs.set('email',c.email);const r=await fetch(NPT_API_BASE+'/api/store/orders?'+qs.toString(),{cache:'no-store'});const d=await r.json();if(!d.ok)return false;const oldOrders=JSON.stringify(S.orders||[]);const oldIds=new Set(myNotifications().map(n=>String(n.id)));const localCustomerNotifs=(S.notifications||[]).filter(n=>n.audience==='customer');const readMap=new Map(localCustomerNotifs.map(n=>[String(n.id),!!n.read]));S.orders=d.orders||[];const incoming=(d.notifications||[]).map(n=>({...n,read:readMap.has(String(n.id))?readMap.get(String(n.id)):!!n.read}));S.notifications=(S.notifications||[]).filter(n=>n.audience!=='customer');S.notifications.push(...incoming);S.notifications.sort((a,b)=>new Date(b.date)-new Date(a.date));lastCustomerSyncAt=Date.now();save();updateCart();const newAlerts=incoming.filter(n=>!oldIds.has(String(n.id))&&!n.read);for(const n of newAlerts)showBrowserAlert(n);const ordersChanged=JSON.stringify(S.orders)!==oldOrders;const drawer=document.getElementById('drawer');const title=document.getElementById('drawerTitle')?.textContent;if(ordersChanged&&(renderOrders||drawer?.classList.contains('open')&&title==='My Orders'))showOrders();if(ordersChanged&&drawer?.classList.contains('open')&&title==='Order details'){const detailId=drawer.dataset.orderId;const fresh=S.orders.find(o=>o.id===detailId);if(fresh)showOrderDetail(fresh.id)}if(newAlerts.length&&drawer?.classList.contains('open')&&title==='Notifications')showNotifications();return true}catch(e){return false}finally{customerSyncBusy=false}}
setInterval(()=>syncLiveCustomer(true),3000);setInterval(()=>syncLiveCatalog(false),8000);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){syncLiveCatalog(false);syncLiveCustomer(true)}});window.addEventListener('focus',()=>syncLiveCustomer(true));
let pushInitialized=false;
async function initBrowserPush(){try{if(pushInitialized)return;if(!('Notification' in window)||!('serviceWorker' in navigator)||!window.firebase)return;const r=await fetch(NPT_API_BASE+'/api/push/config',{cache:'no-store'});const d=await r.json();if(!d.ok||!d.configured)return;const reg=await navigator.serviceWorker.register('/customer-store/firebase-messaging-sw.js');try{await navigator.serviceWorker.ready;await reg.active?.postMessage({type:'NPT_FIREBASE_CONFIG',config:d.config})}catch{}if(Notification.permission==='default'){const ask=confirm('Enable New Pratap Tools notifications for order updates and offers?');if(!ask)return;await Notification.requestPermission()}if(Notification.permission!=='granted')return;if(!firebase.apps.length)firebase.initializeApp(d.config);const messaging=firebase.messaging();const token=await messaging.getToken({vapidKey:d.config.vapidKey,serviceWorkerRegistration:reg});const c=JSON.parse(localStorage.getItem('npt_customer')||'null');if(token&&c){await fetch(NPT_API_BASE+'/api/push/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({token,customer:{mobile:c.mobile,email:c.email}})})}pushInitialized=true;messaging.onMessage(payload=>{const n={id:'local-push-'+Date.now(),type:payload.data?.type||'general',audience:'customer',customerRef:c?.mobile||c?.email,title:payload.notification?.title||'New Pratap Tools',text:payload.notification?.body||'',read:false,date:new Date().toISOString(),orderId:payload.data?.orderId||''};S.notifications=S.notifications||[];S.notifications.unshift(n);save();updateCart();toast(n.title);});}catch(e){console.error('Browser push initialization failed:',e);throw e}
}
function showBrowserAlert(n){try{if(typeof Notification==='undefined'||Notification.permission!=='granted')return;const notification=new Notification(n.title||'New Pratap Tools',{body:n.text||'You have a new update.',icon:'/customer-store/assets/hero-tools.svg',tag:'npt-'+n.id});notification.onclick=()=>{window.focus();if(n.orderId)showOrderDetail(n.orderId);else showNotifications();notification.close()};}catch(e){}}
async function enableDemoChromeNotifications(){if(!('Notification' in window)||!('serviceWorker' in navigator)||!window.firebase)return toast('Chrome push is not supported here');try{let p=Notification.permission;if(p==='default')p=await Notification.requestPermission();if(p!=='granted')return toast('Notification permission was not granted');pushInitialized=false;await initBrowserPush();toast('Chrome notifications enabled')}catch(e){console.error('Push registration failed:',e);toast('Could not enable notifications')}}
...
function updateCart(){const qty=cart.reduce((a,x)=>a+x.qty,0);const cartCount=document.getElementById('cartCount');if(cartCount)cartCount.textContent=qty;const mobileCartBadge=document.getElementById('mobileCartBadge');if(mobileCartBadge){mobileCartBadge.textContent=qty>99?'99+':qty;mobileCartBadge.style.display=qty?'inline-flex':'none'}const unread=myNotifications().filter(n=>!n.read).length;const dot=document.getElementById('notifDot');if(dot)dot.style.display=unread?'block':'none';const badge=document.getElementById('mobileNotifBadge');if(badge){badge.textContent=unread>9?'9+':unread;badge.style.display=unread?'inline-flex':'none'}}
...
function goHome(){closeDrawer();currentCat='All Products';homeShowAll=false;renderHome();window.scrollTo({top:0,behavior:'smooth'})}
...
function closeDrawer(){const d=document.getElementById('drawer'),sh=document.getElementById('shade');if(d)d.classList.remove('open');if(sh)sh.classList.remove('show');document.body.classList.remove('drawer-open','profile-mode');if(d)delete d.dataset.orderId}

/* ================================================================
   VERSION 37 — PREMIUM MOBILE BOTTOM NAVIGATION + CENTRAL API ROUTING
   Reference: floating white pill + raised centre SHOP action.
   Existing app architecture/functions remain unchanged.
   ================================================================ */
function installPremiumMobileNav(){
  ...
  nav.innerHTML=`
    <button class="mobile-nav-item" data-tab="home" type="button" aria-label="Home" onclick="mobileNav('home');goHome()">
      <span class="mobile-nav-icon">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 10.7 12 3.8l8.5 6.9v8.1a1.7 1.7 0 0 1-1.7 1.7H5.2a1.7 1.7 0 0 1-1.7-1.7z"/><path d="M9 20.5v-6.2h6v6.2"/></svg>
      </span>
      <span class="mobile-nav-label">Home</span>
    </button>

    <button class="mobile-nav-item" data-tab="search" type="button" aria-label="Search" onclick="mobileNav('search');mobileSearch()">
      <span class="mobile-nav-icon">
    ...
    <button class="mobile-nav-item mobile-nav-shop" data-tab="shop" type="button" aria-label="Shop" onclick="mobileNav('shop');closeDrawer();scrollToProducts()">
      <span class="mobile-nav-shop-button">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 9h14l-1 11H6L5 9Z"/><path d="M8 9a4 4 0 0 1 8 0"/><path d="M9 13h.01M15 13h.01"/></svg>
      </span>
      <span class="mobile-nav-label">Shop</span>
    </button>

    <button class="mobile-nav-item" data-tab="cart" type="button" aria-label="Cart" onclick="mobileNav('cart');openCart()">
      <span class="mobile-nav-icon mobile-nav-cart-icon">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 4.5h2l1.7 10.1a2 2 0 0 0 2 1.7h8.2a2 2 0 0 0 1.9-1.5L21 8H7"/><circle cx="10" cy="20" r="1.2"/><circle cx="18" cy="20" r="1.2"/></svg>
        <span id="mobileCartBadge" class="mobile-nav-badge">0</span>
      </span>
      <span class="mobile-nav-label">Cart</span>
    </button>

    <button class="mobile-nav-item" data-tab="profile" type="button" aria-label="Profile" onclick="mobileNav('profile');showProfile()">
      <span class="mobile-nav-icon">
        <svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.3"/><path d="M5.2 20.2a6.8 6.8 0 0 1 13.6 0"/></svg>
      </span>
      <span class="mobile-nav-label">Profile</span>
    </button>
  `;

  if(!document.getElementById('npt-v37-mobile-nav-style')){
    const style=document.createElement('style');
    style.id='npt-v37-mobile-nav-style';
    style.textContent=`
      .mobile-nav{
        ...
        box-shadow:0 16px 45px rgba(16,24,40,.16),0 3px 12px rgba(16,24,40,.08) !important;
      ...
      .mobile-nav-item:hover{background:#f8fafc !important}
      .mobile-nav-item.active{color:#101828 !important}
      .mobile-nav-icon{
      ...
      .mobile-nav-item.active .mobile-nav-icon svg{stroke-width:2.2 !important}
      .mobile-nav-shop{
      ...
      .mobile-nav-shop-button{
        ...
        box-shadow:0 9px 22px rgba(16,24,40,.24) !important;
        transition:transform .18s ease,box-shadow .18s ease !important;
      }
      .mobile-nav-shop:active .mobile-nav-shop-button{transform:scale(.94) !important}
      .mobile-nav-shop-button svg{
      ...
      .mobile-nav-shop .mobile-nav-label{
      ...
      @media (max-width:420px){
        .mobile-nav{bottom:10px !important;height:66px !important;border-radius:22px !important;padding:6px 7px !important}
        .mobile-nav-item{height:53px !important}
        .mobile-nav-shop{height:64px !important}
        .mobile-nav-shop-button{width:52px !important;height:52px !important}
        .mobile-nav-label{font-size:9px !important}
      }
      @media (max-width:360px){
        .mobile-nav{width:94vw !important}
  ...
  if(!document.getElementById('npt-v37-nav-final-style')){
    const finalStyle=document.createElement('style');
    finalStyle.id='npt-v37-nav-final-style';
    finalStyle.textContent=`
      .mobile-nav{position:fixed!important;left:50%!important;bottom:max(12px,env(safe-area-inset-bottom))!important;transform:translateX(-50%)!important;width:min(92vw,520px)!important;height:76px!important;padding:7px 9px!important;display:grid!important;grid-template-columns:repeat(5,1fr)!important;gap:2px!important;align-items:end!important;background:rgba(255,255,255,.98)!important;border:1px solid rgba(15,23,42,.08)!important;border-radius:25px!important;box-shadow:0 18px 50px rgba(15,23,42,.18),0 3px 12px rgba(15,23,42,.08)!important;backdrop-filter:blur(18px)!important;-webkit-backdrop-filter:blur(18px)!important;z-index:120!important;box-sizing:border-box!important;overflow:visible!important}
      .mobile-nav-item{position:relative!important;height:60px!important;min-width:0!important;padding:6px 2px!important;margin:0!important;border:0!important;background:transparent!important;color:#667085!important;display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:center!important;gap:5px!important;border-radius:16px!important;font:inherit!important;cursor:pointer!important;overflow:visible!important}
      .mobile-nav-item:hover{background:transparent!important}
      .mobile-nav-item.active{color:#0b3154!important;background:transparent!important}
      .mobile-nav-item.active:after{content:''!important;position:absolute!important;top:1px!important;left:50%!important;width:5px!important;height:5px!important;transform:translateX(-50%)!important;border-radius:50%!important;background:#0b3154!important}
      .mobile-nav-icon{width:26px!important;height:26px!important;display:grid!important;place-items:center!important;position:relative!important;background:transparent!important;border-radius:0!important}
      .mobile-nav-icon svg{width:22px!important;height:22px!important;fill:none!important;stroke:currentColor!important;stroke-width:1.8!important;stroke-linecap:round!important;stroke-linejoin:round!important}
      .mobile-nav-label{display:block!important;width:auto!important;height:auto!important;background:transparent!important;border-radius:0!important;font-size:10px!important;line-height:1!important;font-weight:750!important;letter-spacing:.01em!important;color:inherit!important}
      .mobile-nav-item.active .mobile-nav-icon svg{stroke-width:2.25!important}
      .mobile-nav-shop{height:78px!important;margin-top:-27px!important;padding:0 2px 3px!important;justify-content:flex-end!important;z-index:3!important}
      .mobile-nav-shop:after{display:none!important}
      .mobile-nav-shop-button{width:58px!important;height:58px!important;margin:0!important;display:grid!important;place-items:center!important;border-radius:50%!important;background:#0b3154!important;color:#fff!important;border:5px solid #fff!important;box-shadow:0 11px 26px rgba(11,49,84,.30)!important;transform:translateY(-3px)!important}
      .mobile-nav-shop-button svg{width:25px!important;height:25px!important;fill:none!important;stroke:currentColor!important;stroke-width:1.8!important;stroke-linecap:round!important;stroke-linejoin:round!important}
      .mobile-nav-shop .mobile-nav-label{color:#0b3154!important;font-weight:850!important;margin-top:0!important}
      .mobile-nav-shop.active .mobile-nav-shop-button{background:#101828!important;box-shadow:0 12px 28px rgba(16,24,40,.32)!important}
      .mobile-nav-badge{top:-3px!important;right:-6px!important}
      .mobile-nav-item:active .mobile-nav-icon{transform:scale(.94)!important}
      .mobile-nav-shop:active .mobile-nav-shop-button{transform:translateY(-3px) scale(.95)!important}
      @media(min-width:801px){.mobile-nav{display:none!important}}
      @media(max-width:420px){.mobile-nav{width:94vw!important;height:72px!important;border-radius:23px!important;padding:6px 7px!important}.mobile-nav-item{height:57px!important}.mobile-nav-shop{height:75px!important;margin-top:-25px!important}.mobile-nav-shop-button{width:54px!important;height:54px!important}.mobile-nav-label{font-size:9px!important}}
    `;
...
function openCart(){document.body.classList.remove('profile-mode');openDrawer();document.getElementById('drawerTitle').textContent='Your Cart';renderCart()}
...
/* ============================================================
   ...
   The existing customer-store architecture is preserved.
   Demo authentication is local so Firebase/SMS/email providers
   are NOT required for testing.
   ============================================================ */

// DEMO AUTH MODE
// 1234 is intentionally fixed for local/demo testing.
// No Firebase/SMS/email credentials are required for demo login.
// Live OTP can be enabled later through the existing /api/auth endpoints.
const DEMO_OTP_MODE = true;
const DEMO_OTP_CODE = '1234';
const DEMO_OTP_EXPIRY_MS = 5 * 60 * 1000;

let demoOtpSession = null;

function showAuth(mode='login'){
  const old=document.getElementById('authModal');
  if(old)old.remove();

  document.body.insertAdjacentHTML('beforeend',`<div class="modal" id="authModal"><div class="auth-card"><button class="closex" onclick="document.getElementById('authModal').remove()">×</button><div class="kicker">NEW PRATAP TOOLS</div><h2>${mode==='signup'?'Create your account':'Sign in securely'}</h2><p class="muted">Verify your mobile or email with a one-time code. No password required.</p><div class="auth-tabs"><button class="${mode==='login'?'active':''}" onclick="showAuth('login')">Sign in</button><button class="${mode==='signup'?'active':''}" onclick="showAuth('signup')">Create account</button></div><div class="form">${mode==='signup'?'<input id="aname" placeholder="Full name" autocomplete="name">':''}<input id="amobile" inputmode="tel" placeholder="Mobile number" autocomplete="tel"><input id="aemail" type="email" placeholder="Email address" autocomplete="email"></div><div class="auth-methods"><button class="auth-method" onclick="requestLoginOtp('${mode}','sms')" type="button"><span class="method-icon mobile-mark">M</span><span><b>Mobile number</b><small>${DEMO_OTP_MODE?'Demo verification code':'Receive a verification code'}</small></span><strong>Continue</strong></button><button class="auth-method" onclick="requestLoginOtp('${mode}','email')" type="button"><span class="method-icon email-mark">@</span><span><b>Email address</b><small>${DEMO_OTP_MODE?'Demo verification code':'Receive a verification code'}</small></span><strong>Continue</strong></button></div><button class="textbtn" onclick="document.getElementById('authModal').remove()">Continue as guest</button></div></div>`);
}

async function requestLoginOtp(mode,channel){
  const mobile=(document.getElementById('amobile')?.value||'').replace(/\D/g,'').slice(-10);
  const email=(document.getElementById('aemail')?.value||'').trim().toLowerCase();

  if(channel==='sms' && mobile.length!==10)return toast('Enter a valid 10-digit mobile number');
  if(channel==='email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))return toast('Enter a valid email address');

  if(DEMO_OTP_MODE){
    demoOtpSession={mode,channel,mobile,email,code:DEMO_OTP_CODE,createdAt:Date.now(),expiresAt:Date.now()+DEMO_OTP_EXPIRY_MS};
    document.querySelector('#authModal .otp-verify')?.remove();
    const card=document.querySelector('#authModal .auth-card');
    if(!card)return;
    card.insertAdjacentHTML('beforeend',`<div class="otp-verify"><div style="padding:12px 14px;margin-bottom:12px;border-radius:10px;background:#fff8df;border:1px solid #f5d36b;color:#6b4f00;font-size:13px"><b>Demo verification</b><br><span>Your demo OTP is <strong style="letter-spacing:4px">1234</strong></span></div><div class="form"><input id="otpCode" inputmode="numeric" maxlength="4" pattern="[0-9]{4}" placeholder="Enter demo OTP" autocomplete="one-time-code" aria-label="Demo OTP"></div><button class="checkout" onclick="verifyLoginOtp('${mode}','${channel}')" type="button">Verify & continue</button><button type="button" class="secondary" style="width:100%;margin-top:8px" onclick="requestLoginOtp('${mode}','${channel}')">Send code again</button><small style="display:block;margin-top:10px;color:#667085">Demo mode: use <b>1234</b>. No SMS or email delivery is required.</small></div>`);
    setTimeout(()=>document.getElementById('otpCode')?.focus(),50);
    toast(channel==='sms'?'Demo OTP ready for mobile':'Demo OTP ready for email');
    return;
  }

  try{
    const r=await fetch(NPT_API_BASE+'/api/auth/request-otp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mobile,email,channel})});
    const d=await r.json();
    if(!d?.ok)return toast(d?.error||'Could not send OTP');
    document.querySelector('#authModal .otp-verify')?.remove();
    const card=document.querySelector('#authModal .auth-card');
    card.insertAdjacentHTML('beforeend',`<div class="otp-verify"><div class="form"><input id="otpCode" inputmode="numeric" maxlength="6" placeholder="Enter OTP" autocomplete="one-time-code"></div><button class="checkout" onclick="verifyLoginOtp('${mode}','${channel}')" type="button">Verify & continue</button><small>Code expires in 5 minutes.</small></div>`);
    toast(d.demoCode?'Demo OTP: 1234':'OTP sent');
  }catch(e){console.error('OTP request error:',e);toast('Unable to contact authentication server');}
}

async function verifyLoginOtp(mode,channel){
  const mobile=(document.getElementById('amobile')?.value||'').replace(/\D/g,'').slice(-10);
  const email=(document.getElementById('aemail')?.value||'').trim().toLowerCase();
  ...
  if(DEMO_OTP_MODE){
    // Demo mode deliberately accepts the fixed code without a client-side
    // expiry/session dependency. This prevents the previous
    // "OTP expired" problem after refresh, delayed testing, or UI re-render.
    if(code!==DEMO_OTP_CODE)return toast('Incorrect OTP. Use 1234.');

    const existingCustomer=JSON.parse(localStorage.getItem('npt_customer')||'null');
    const customer={id:existingCustomer?.id||'CUS-'+Date.now(),name:name||existingCustomer?.name||'Customer',mobile:mobile||existingCustomer?.mobile||'',email:email||existingCustomer?.email||'',city:existingCustomer?.city||'Ghaziabad',address:existingCustomer?.address||'',pincode:existingCustomer?.pincode||'',createdAt:existingCustomer?.createdAt||new Date().toISOString(),lastLoginAt:new Date().toISOString()};

    localStorage.setItem('npt_customer',JSON.stringify(customer));
    S=NPT.load();
    S.customers=S.customers||[];
    const nm=String(customer.mobile||'').replace(/\D/g,'');
    const ne=String(customer.email||'').trim().toLowerCase();
    const i=S.customers.findIndex(x=>(nm&&String(x.mobile||'').replace(/\D/g,'')===nm)||(ne&&String(x.email||'').trim().toLowerCase()===ne));
    if(i>=0)S.customers[i]={...S.customers[i],...customer};else S.customers.push(customer);
    save();
    demoOtpSession=null;
    document.getElementById('authModal')?.remove();
    document.getElementById('welcome')?.classList.add('hide');
    const accountText=document.getElementById('accountText');
    if(accountText)accountText.textContent=(customer.name||'Account').split(' ')[0];
    try{await syncLiveCustomer(false)}catch(e){console.warn('Customer sync skipped:',e)}
    try{await initBrowserPush()}catch(e){console.warn('Browser push unavailable:',e)}
    toast('Welcome, '+(customer.name||'Customer').split(' ')[0]);
    return;
  }

  if(!/^\d{4,6}$/.test(code))return toast('Enter the OTP you received');
  try{
    const r=await fetch(NPT_API_BASE+'/api/auth/verify-otp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mobile,email,code,channel,mode,name})});
    const d=await r.json();
    if(!d?.ok)return toast(d?.error||'Verification failed');
    if(!d.customer)return toast('Customer account was not returned');
    localStorage.setItem('npt_customer',JSON.stringify(d.customer));
    S=NPT.load();S.customers=S.customers||[];
    const i=S.customers.findIndex(x=>String(x.mobile||'').replace(/\D/g,'')===String(d.customer.mobile||'').replace(/\D/g,''));
    if(i>=0)S.customers[i]=d.customer;else S.customers.push(d.customer);
    save();
    document.getElementById('authModal')?.remove();
    document.getElementById('welcome')?.classList.add('hide');
    const accountText=document.getElementById('accountText');
    if(accountText)accountText.textContent=(d.customer.name||'Account').split(' ')[0];
    await syncLiveCustomer(false);await initBrowserPush();
    toast('Welcome, '+(d.customer.name||'Customer').split(' ')[0]);
  }catch(e){console.error('OTP verification error:',e);toast('Unable to verify OTP');}
}

async function quickLogin(){
  const mobile=(document.getElementById('welcomeMobile')?.value||'').replace(/\D/g,'').slice(-10);
  if(mobile.length!==10)return toast('Enter a valid 10-digit mobile number');
  showAuth('login');
  setTimeout(()=>{const x=document.getElementById('amobile');if(x)x.value=mobile},30);
}
...
installPremiumMobileNav();catNav();renderHome();updateCart();syncLiveCatalog(true);
