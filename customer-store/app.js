...
function updateCart(){document.getElementById('cartCount').textContent=cart.reduce((a,x)=>a+x.qty,0);document.getElementById('notifDot').style.display=myNotifications().some(n=>!n.read)?'block':'none'}
...
function showAuth(mode='login'){
 ...
 const existing=document.getElementById('authModal');if(existing)existing.remove();
 document.body.insertAdjacentHTML('beforeend',`<div class="modal" id="authModal"><div class="auth-card"><button class="closex" onclick="document.getElementById('authModal').remove()">×</button><div class="kicker">NEW PRATAP TOOLS</div><h2>${mode==='signup'?'Create your account':'Sign in with mobile'}</h2><p class="muted">${mode==='signup'?'Save your orders, wishlist and details.':'Enter your mobile number to access your saved account.'}</p><div class="auth-tabs"><button class="${mode==='login'?'active':''}" onclick="showAuth('login')">Sign in</button><button class="${mode==='signup'?'active':''}" onclick="showAuth('signup')">Create account</button></div><div class="form">${mode==='signup'?'<input id="aname" placeholder="Full name">':''}<input id="amobile" inputmode="tel" placeholder="Mobile number">${mode==='signup'?'<input id="aemail" placeholder="Email (optional)">':''}${mode==='signup'?'<input id="apass" type="password" placeholder="Password">':''}</div><button class="checkout" onclick="${mode==='signup'?'createAccount()':'signIn()'}">${mode==='signup'?'Create account':'Continue with mobile'}</button><button class="textbtn" onclick="document.getElementById('authModal').remove()">Continue as guest</button></div></div>`);
}
async function createAccount(){
 let name=document.getElementById('aname').value.trim(),mobile=document.getElementById('amobile').value.trim(),email=document.getElementById('aemail')?.value.trim()||'';
 if(!name||!mobile)return toast('Enter your name and mobile number');
 let c={name,mobile,email,created:new Date().toISOString()};
 try{const r=await fetch((window.NPT_API_BASE||'')+'/api/store/customer',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({customer:c})});const d=await r.json();if(d.ok)c=d.customer||c}catch(e){}
 localStorage.setItem('npt_customer',JSON.stringify(c));S.customers=S.customers||[];
 S.customers=S.customers.filter(x=>String(x.mobile||'').replace(/\D/g,'')!==String(mobile).replace(/\D/g,''));
 S.customers.push(c);S.notifications=S.notifications||[];
 S.notifications.unshift({id:'n'+Date.now(),type:'customer',audience:'admin',title:'New customer signup',text:`${name} joined from the customer store.`,read:false,date:new Date().toISOString()});
 save();document.getElementById('authModal').remove();document.getElementById('accountText').textContent=name.split(' ')[0];toast('Account created');
}
async function signIn(){
 const mobile=(document.getElementById('amobile').value||'').replace(/\D/g,'');
 if(!mobile)return toast('Enter your mobile number');
 try{const r=await fetch((window.NPT_API_BASE||'')+'/api/store/customer?mobile='+encodeURIComponent(mobile),{cache:'no-store'});const d=await r.json();if(d.ok&&d.customer){const c=d.customer;localStorage.setItem('npt_customer',JSON.stringify(c));S.customers=S.customers||[];const i=S.customers.findIndex(x=>String(x.mobile||'').replace(/\D/g,'')===mobile);if(i>=0)S.customers[i]=c;else S.customers.push(c);save();document.getElementById('authModal').remove();document.getElementById('accountText').textContent=c.name.split(' ')[0];await syncLiveCustomer(false);toast('Signed in');return}}catch(e){}
 const c=(S.customers||[]).find(x=>String(x.mobile||'').replace(/\D/g,'').endsWith(mobile.slice(-10)));
 if(!c)return toast('Account not found — create an account or continue as guest');
 localStorage.setItem('npt_customer',JSON.stringify(c));document.getElementById('authModal').remove();document.getElementById('accountText').textContent=c.name.split(' ')[0];toast('Signed in');
}
...
function showNotifications(){openDrawer();document.getElementById('drawerTitle').textContent='Notifications';let ns=myNotifications();document.getElementById('drawerBody').innerHTML=ns.length?ns.slice(0,15).map(n=>`<button class="customer-notification ${n.read?'':'unread'}" onclick="openCustomerNotification('${n.id}')"><span class="notification-icon">${n.type==='order'?'📦':n.type==='stock'?'⚡':'🔔'}</span><span class="notification-copy"><b>${n.title}</b><small>${n.text}</small><em>${new Date(n.date).toLocaleString()}</em></span><span class="notification-arrow">›</span></button>`).join(''):`<div class="empty">You're all caught up.</div>`}
...
function openNotifications(){showNotifications()}
...
if(logged){document.getElementById('accountText').textContent=(logged.name||'Account').split(' ')[0];refreshCustomerProfile();syncLiveCustomer(false)}
